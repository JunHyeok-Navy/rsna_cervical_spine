...
def train():
    for fold in range(config.N_FOLDS):
        ...
        train_loader = torch.utils.data.DataLoader(train_data, batch_size=config.BATCH_SIZE, shuffle=True, pin_memory=True, num_workers=8)
        valid_loader = torch.utils.data.DataLoader(valid_data, batch_size=config.BATCH_SIZE, shuffle=False, pin_memory=True, num_workers=8)
    ...
    # Save History
    with open('final_loss_hist.pkl', 'wb') as f:
        pickle.dump(final_loss_hist, f)
    with open('final_val_loss_hist.pkl', 'wb') as f:
        pickle.dump(final_val_loss_hist, f)
    with open('best_val_loss_list.pkl', 'wb') as f:
        pickle.dump(best_val_loss_list, f)
